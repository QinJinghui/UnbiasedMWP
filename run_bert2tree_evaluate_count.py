...
if __name__ == "__main__":
    ...
    all_len = {'3':0, '4':0, '5':0, '6':0 ,'7':0, '8':0, '9':0, '10':0, '11':0, 'else':0}
    acc_len = {'3':0, '4':0, '5':0, '6':0 ,'7':0, '8':0, '9':0, '10':0, '11':0, 'else':0}

    for test_batch in tqdm(test_pairs):
        key = len(test_batch['output'])
        if key <= 11:
            key = str(key)
        else:
            key = 'else'
    ...
    for key in all_len:
        if all_len[key] != 0:
            acc_len[key] = acc_len[key]/all_len[key]    
    print(acc_len)
